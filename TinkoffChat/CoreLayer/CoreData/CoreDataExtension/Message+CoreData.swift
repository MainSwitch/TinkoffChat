...
extension Message {
    static func generateMessageId() -> String {
        let string =
        "\(arc4random_uniform(UInt32.max)) + \(Date.timeIntervalSinceReferenceDate) + \(arc4random_uniform(UInt32.max))"
    ...
    static func insertMessage(messageText: String,
                              recieverId: String,
                              senderId: String,
                              in context: NSManagedObjectContext) -> Message? {
    ...
    static func findMessagesByConversation(with conversationId: String,
                                           in context: NSManagedObjectContext) -> [Message]? {
        ...
        guard let fetchRequest = Message.fetchRequestMessageByConversation(with: conversationId, model: model) else {
            return nil
        }
        do {
    ...
    static func fetchRequestMessageByConversation(with conversationId: String,
        ...
        guard let fetchRequest = model.fetchRequestFromTemplate(withName: templateName,
            ...
            ["conversationId": conversationId]) as? NSFetchRequest<Message> else {
