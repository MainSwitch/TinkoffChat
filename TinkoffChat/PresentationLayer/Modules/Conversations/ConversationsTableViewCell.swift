        ...
        didSet {
            if let messageValue = message {
                self.messageLabel.attributedText =
                    NSAttributedString(string: messageValue,
                                       attributes: [NSAttributedString.Key.font:
                                        UIFont.systemFont(ofSize:
                                            self.messageLabel.font.pointSize, weight: UIFont.Weight.light)])
            } else {
                self.messageLabel.attributedText =
                    NSAttributedString(string: "No messages yet",
                                       attributes: [NSAttributedString.Key.font:
                                        UIFont.italicSystemFont(ofSize:
                                            self.messageLabel.font.pointSize)])
            }
        }
    }
    var date: Date? {
    ...
    var hasUnreadedMessages: Bool = false {
        didSet {
            if hasUnreadedMessages {
                self.messageLabel.attributedText =
                    NSAttributedString(string: self.messageLabel.text!,
                                       attributes: [NSAttributedString.Key.font:
                                        UIFont.systemFont(ofSize: self.messageLabel.font.pointSize, weight:
                                            UIFont.Weight.bold)])
            }
        }
    }
