...
class CommunicationManager: CommunicationDelegate {
    ...
    func didLostUser(userID: String) {
        for (index, user) in foundUser.enumerated() {
            for (index, model) in connectedPeer.enumerated() where user.userID == userID &&
                model.peerID.displayName == userID {
    ...
    func failedToStartBrowsingForUser(error: Error) {
        conversationsListViewController.showAlertWith(title: "Failed to start browsing for user",
                                                      message: "попробуйте перезагрузить приложение",
                                                      completion: nil)
    }
    func failedToStartAdvertising(error: Error) {
        conversationsListViewController.showAlertWith(title: "failed to start advertising",
                                                      message: "попробуйте перезагрузить приложение",
                                                      completion: nil)
    }
            ...
            for (index, model) in conversationsPresenter.messageModelArray.enumerated()
                ...
                for (index, model) in conversationsPresenter.messageConversationFrom.enumerated() {
                    ...
                    conversationsPresenter.messageModelArray.insert(MessageModel(name: fromUser,
                                                                                 ...
                                                                                 date: Date(),
                                                                                 online: true,
                                                                                 hasUnreadMessages: true), at: index)
                    DispatchQueue.main.async {
    ...
    func invitationWasReceived(fromPeer: String) {
        let alert = UIAlertController(title: "",
                                      message: "\(fromPeer) wants to chat with you.",
            preferredStyle: UIAlertController.Style.alert)
        let acceptAction: UIAlertAction = UIAlertAction(title: "Accept",
    ...
    func connectedWithPeer(peerID: MCPeerID, session: MCSession) {
        var name: String {
            var string: String!
            for user in foundUser where user.userID == peerID.displayName {
        ...
        var notContain = true
        for conectedModel in connectedPeer where conectedModel.peerID == peerID {
            notContain = false
        }
        if notContain {
    ...
    func didNotConnect(toPeer: String) {
        ...
        for (index, conectedModel) in connectedPeer.enumerated()
            where conectedModel.peerID.displayName == toPeer {
            notContain = false
            removeIndex = index
        }
        if !notContain {
